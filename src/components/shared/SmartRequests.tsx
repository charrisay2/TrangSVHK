...
export default function SmartRequests() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const type = user?.role === 'STUDENT' ? 'STUDENT_LEAVE' : 'TEACHER_SUBSTITUTE';
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'PENDING': return <span className="px-2 py-1 bg-amber-100 text-amber-700 rounded-lg text-xs font-bold flex items-center gap-1 w-fit"><Clock size={12}/> Chờ duyệt</span>;
      case 'APPROVED': return <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold flex items-center gap-1 w-fit"><CheckCircle size={12}/> Đã duyệt</span>;
      case 'REJECTED': return <span className="px-2 py-1 bg-rose-100 text-rose-700 rounded-lg text-xs font-bold flex items-center gap-1 w-fit"><XCircle size={12}/> Đã từ chối</span>;
      default: return null;
    }
  };
    ...
    <div className="space-y-6">
        ...
        {showForm && (
          ...
          >
            <h2 className="text-lg font-bold text-slate-800 mb-4">
              {user?.role === 'STUDENT' ? 'Đơn xin nghỉ phép' : 'Đơn báo bận / Dạy thay'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Căn cứ (Mã lớp học / Học phần)</label>
                <input 
                  type="text" 
                  value={targetClassId} 
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                  Hủy
                </button>
                <button type="submit" className="px-4 py-2 bg-indigo-600 text-white font-bold tracking-wide rounded-xl shadow-md shadow-indigo-200 hover:bg-indigo-700 transition-colors">
                  Gửi đơn
                </button>
              </div>
            </form>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-500">Chưa có đơn từ nào.</div>
        ) : (
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <h3 className="font-bold text-slate-800">
                      {req.type === 'STUDENT_LEAVE' ? 'Đơn Xin Nghỉ Phép' : 'Đơn Báo Bận / Dạy Thay'}
                    </h3>
                    {getStatusBadge(req.status)}
                  </div>
                  <p className="text-sm text-slate-600"><span className="font-medium text-slate-700">Người gửi:</span> {req.requester.name} - {req.requester.username}</p>
                  {req.targetClass && (
